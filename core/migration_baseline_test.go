...
func TestMigrationBaselineLaterFailure(t *testing.T) {
	...
	a := New(p).WithTools(Func("echo", "echo", func(context.Context, echoArgs) (string, error) { return "ok", nil }))
...
func TestMigrationBaselineTypedAccountingReset(t *testing.T) {
	type answer struct {
	...
	v, res, err := RunTyped[answer](context.Background(), New(p).WithMaxSteps(1), "go")
	if err != nil || v.Value != "ok" || p.calls != 2 || res.Steps != 1 || res.Usage.InputTokens != 8 {
...
func TestMigrationBaselinePartialToolJSON(t *testing.T) {
	...
	res, err := New(p).Run(context.Background(), "go")
	if !errors.Is(err, ErrIncompleteResponse) || len(res.Messages) != 2 {
		t.Fatalf("result=%+v err=%v", res, err)
	}
	if _, err := json.Marshal(res.Messages); err == nil {
		t.Fatal("baseline partial history unexpectedly marshals")
	}
}

func TestMigrationBaselineZeroOptionsInherit(t *testing.T) {
	p := &optionsProvider{turns: []Message{asstText("ok")}}
	_, err := New(p).WithDefaultCallOptions(CallOptions{MaxTokens: 32, ThinkingBudget: 16}).Run(context.Background(), "go", WithCallOptions(CallOptions{}))
